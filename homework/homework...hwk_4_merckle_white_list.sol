...
contract ERC20 is IERC20 {
    ...
    constructor(string memory _name,string memory _sym, uint8 _decimals){
        ...
        merckleRoot = 0x5e0020c213f92fac3ea365575ef8c29ed0f5bc0f7b8b02b42cb8100b5781c4eb;
    }
    ...
    function isWhiteList(address _who,bytes32[] memory merckleProof)public view returns(bool){
        bytes32 proof = sha256(abi.encodePacked(_who));
        for (uint256 i;i<merckleProof.length;i++){
            if (proof > merckleProof[i]){
                proof = sha256(abi.encodePacked(proof,merckleProof[i]));
            }else{
                proof = sha256(abi.encodePacked(merckleProof[i],proof));
            }
        }
        return proof == merckleRoot;
    }

    function mint(bytes32[] memory merckleProof) public returns (bool){
        require(isWhiteList(msg.sender, merckleProof),"proof fails");
